...
public class FormalExpressionTreeImpl implements FormalExpressionTree {
	...
	public FormalExpressionTree derive() {
		FormalExpressionTreeImpl tDerive = new FormalExpressionTreeImpl(
				Calculator.derive(lbt));
		return tDerive;
	}
		...
		for (int i = 0; i < size; i++) {
			...
			if (isOperator(token) || fnct != -1) {
				...
				if (fnct == FUNCTION_COS) {
					...
					LinkedBinaryTree lbt = new LinkedBinaryTree(operators.pop()
							+ "", left, right);
					operands.push(lbt);
				} else if (token == '(') {
					continue;
				...
				if (Character.isDigit(expression.charAt(i))) {
					for (int j = i; j < size; j++) {
						if (Character.isDigit(expression.charAt(j))) {
							...
							operands.push(new LinkedBinaryTree(operand + "",
									null, null));
							i = j - 1;
							break;
						}
